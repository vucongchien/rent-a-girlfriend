...
- [ ] **Infrastructure Setup:** Cấu hình Kubernetes (Local: Minikube/K3s), Docker Registry.
...
## 🛠️ CHIẾN THUẬT TRIỂN KHAI (TECH STRATEGY)

| Thành phần | Công nghệ Đề xuất | Ghi chú |
| :--- | :--- | :--- |
| **Microservice Framework** | Node.js (NestJS) hoặc Go | Tùy vào độ phức tạp của logic nghiệp vụ. |
| **Communication** | gRPC (Internal Sync), RabbitMQ/Kafka (Async) | Tuân thủ CloudEvents JSON format. |
| **Database** | PostgreSQL (Finance/Booking), MongoDB (Profile/Chat) | Database-per-service. |
| **Auth** | Istio Ambient + Identity Service | **Auth Offloading:** Code ứng dụng không tự verify JWT. |
| **Storage** | MinIO / AWS S3 | Dùng Presigned URL để tối ưu traffic. |

---

## ⚠️ CÁC RỦI RO CẦN LƯU Ý (RISKS)
1. **SAGA Complexity:** Luồng hoàn tiền khi có Dispute đòi hỏi logic bù trừ (Compensating transactions) cực kỳ chính xác để tránh thất thoát Kano-Coin.
2. **Real-time Scale:** Notification SSE cần quản lý kết nối tốt khi lượng user tăng cao.
3. **Istio Learning Curve:** Việc vận hành Istio Ambient (ZTunnel/Waypoint) đòi hỏi kỹ năng DevOps tốt.

> [!TIP]
> Nên ưu tiên hoàn thiện **Giai đoạn 1 & 2** thật chuẩn chỉnh vì đây là "xương sống" cho việc phát triển song song các service ở các giai đoạn sau.
